...
def calibrator(mkt_data, lookback_window, lookfwd_window, lookback_threshold, baseline_stock):
     ...
     for ind in winning_stocks.index:
          ...
          print(list_of_gains)
          ########## need to add a clause that allows for null lists of wiinners
     # print(list_of_gains)
     ...
     winning_stocks['pct_gain'] = winning_stocks.apply(lambda x: np.nanmean(x['gains']), axis = 1) #[mean, stdev]
     winning_stocks['stdev'] = winning_stocks.apply(lambda x: np.std(x['gains']), axis = 1) #[mean, stdev]
     return winning_stocks


winning_stocks = calibrator(dow_historical_data, 6,1,6.0,'spy')


def generate_buy_orders(start_date, calibrator_file):
     return 0
